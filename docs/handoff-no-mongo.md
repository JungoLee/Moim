...
## 미결이었던 것 (확인 결과 — 2026-08-19)

- `backend/` 의 데이터 이전이 실제로 끝났는지 → **끝났다** (2026-08-13, 220행 유실 0 — [cf-migration.md](cf-migration.md)). `backend/`+`render.yaml` 삭제는 PLAN.md "다음 작업" 항목으로 관리 중(사용자 확인 후 진행).
- Moim 이 쓰던 Atlas 클러스터가 아직 살아있는지 → **이미 삭제됐다** (2026-08-13, `Cluster0`). 원본 스냅샷은 로컬 `backup/*.json` 뿐이다.
