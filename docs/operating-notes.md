...
## 1. 이메일 코드 로그인 — 이제 실제로 발송된다 (2026-08-18)

**Resend**(`RESEND_API_KEY`, 발신 `noreply@opnae.com`)로 실제 발송한다. 순서대로 해결했다:
① IP 당 하루 10통 레이트 리밋(`worker/auth.js` `overMailRate`, D1 `mail_rate`) → ② Resend 키 등록.

- 무료 쿼터는 하루 100통(Resend). 키는 opnae 프로젝트 공용 — 유출 시 전 프로젝트 키를 함께 교체한다
- 발송이 안 되면: `npx wrangler tail` 로 `[mail]` 줄 확인 (키가 없으면 코드가 로그에만 찍히는 폴백은 그대로다)
- 코드 규격은 **영문 대소문자+숫자 6자, 대소문자 구분**(2026-08-21). 사용자 문의 시 "소문자/대문자 그대로 입력"을 안내한다 — 화면은 6칸 입력이라 붙여넣기 한 번으로 채워진다
...
```powershell
# 실시간 로그 (에러가 여기 찍힌다. 메일 코드는 키가 없는 환경에서만 찍힌다 — 운영은 실발송)
npx wrangler tail
...
데이터가 **440KB, 사용자 5명**(2026-08-13 이관 직후 기준) 이라 한도의 0.01% 수준이다. 정확한 사용량은 Cloudflare 대시보드 → Workers & Pages → moim 에서 볼 수 있다.
