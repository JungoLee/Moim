...
> 살아있는 할 일/상태·기능 로드맵·데이터 모델은 **[docs/PLAN.md](docs/PLAN.md)** 가 단일 출처. 리팩토링 절차는 **[docs/refactoring-guide.md](docs/refactoring-guide.md)**, 셋업·트러블슈팅은 **[docs/ONBOARDING.md](docs/ONBOARDING.md)**, 배포 후 운영 규칙·주의사항은 **[docs/operating-notes.md](docs/operating-notes.md)**, 실행 요약은 **[README.md](README.md)** 참조.
...
- **단일 출처 = `frontend/src/release/releases.ts`** (배열 맨 앞이 최신). DB 가 아니라 소스에 두는 이유는 **배포 산출물과 버전이 함께 고정**돼야 "지금 보는 화면의 릴리즈 노트" 가 맞기 때문이다. 정적 export 라 더욱 — 빌드 결과물에 같이 실려야 한다.
- 노출 위치: 계정 드로어 하단(약관·개인정보 처리방침과 같은 자리) → `ReleaseModal`.
- **배포 직전에 갱신한다.** 지난 릴리즈 이후 커밋을 훑어 **사용자가 체감하는 변화만** 옮긴다.
- 쓰는 법(사용자가 읽는 글이다 — 개발 로그가 아니다):
  - 내부 용어·파일명·함수명 금지. "AvailabilityCalendar 리렌더 최적화"(X) → "빈 시간 찾기가 빨라졌어요"(O)
  - 리팩토링·문서 정리·내부 구조 변경은 **넣지 않는다**
  - 한 항목은 한 줄, `kind` 는 `feature`·`improve`·`fix`. 날짜는 `YYYY-MM-DD`

- **버전 표기** — 릴리즈마다 `major.minor.patch`. 그 릴리즈에서 **가장 큰 변화 기준으로 한 칸만** 올리고 아래 자리는 0 으로 되돌린다.
  - `major` 전면 개편·디자인 변경 · `minor` 기능 추가 · `patch` 버그 수정·다듬기
  - 목록에는 버전이 맨 앞에, 상세에는 상단에 크게 나온다.

## 작업 기록 (.agent-history)
- **17시 이후**이거나 세션을 종료·이동할 때는(하루 1회) 오늘 작업을 아래 규칙으로 기록한다. 시각이 불확실하면 명령으로 확인한다.
- 기록 위치: `C:/.agent-history/daily/{YYYY-MM-DD}/moim.md` — 파일이 있으면 **append**, 절대 덮어쓰지 않는다. 폴더가 없으면 만든다.
- 양식: `C:/.agent-history/WORKLOG-TEMPLATE.md`. 요구사항 원문은 받은 그대로 보존하고, 요청 출처(고객 원문/사내 전달/내가 정리/AI 추론)와 해석·보완을 분리해 적는다.
- 쓰지 말 것(중앙 기록에만 적용): 고객사명·기획 의도·화면 내용·카피 문구, 키·토큰·계정·개인정보.
- 기록 후 `C:/.agent-history` 저장소에 커밋한다 (push는 사용자 확인 후).

## 주간보고 (weekly-report — 보고용, 프로젝트 내부)
- 17시 기록 때 `weekly-report/daily/{YYYY-MM-DD}.md` 에도 오늘 이 프로젝트에서 한 일을 **구체적으로** 적는다 (git 제외 폴더라 자유롭게).
- **매주 목요일**: `weekly-report/daily/` 의 지난 한 주치를 참고해 `weekly-report/weekly/{YYYY-MM-DD}.md` 를 작성한다 — 사용자가 복사해 상급자에게 메일로 보내는 보고문.
- 이 폴더는 `.gitignore` 등재 — 커밋하지 않는다.
